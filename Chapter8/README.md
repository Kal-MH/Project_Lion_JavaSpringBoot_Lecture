...
이렇게 다른 서비스로 사용자 정보를 안전하게 전달하기 위해 OAuth라는 규칙이 정해졌다.

1. 사용자가 클라이언트(서비스)에 접속
2. 클라이언트가 로그인을 요구
3.  로그인을 위한 인증 정보를 클라이언트를 통해서 oauth auth에 제공하기를 요청한다.
4. oauth auth에서 로그인을 진행하고 토큰을 발행한다.
5. 이후, oauth resource로부터 유저 resource가 필요할 때, 어플리케이션은 해당 토큰을 가지고 정보를 받길 요청한다.

사실 소셜 로그인은 로그인 기능이 이뤄지는 것 이라기 보단, 정보를 갖고 있는 원소유주에게 정보를 요청하는 과정이라고 말할 수 있다.

따라서, 해당 정보를 받고 나서는 받은 유저 정보를 관리하는 부분이 필요하다.

또한, 실제 동작과정을 보면 클라이언트에서 로그인이 진행되기 보다는 해당 로그인 사이트로 리다이렉트해서 진행되게 된다. 사용자가 로그인하길 원한다는 정보를 url의 query parameter와 같은 형태로 oauth auth에 전달하게 된다.

또한, 어플리케이션의 callback url을 통해서 이후의 정보를 받을 때 해당 url로 받게 된다.
          ...
          client:
            registration:
              ...
              naver:
                ...
                # 단순 로그인뿐만 아니라 다른 형태로 사용자 정보를 받아오는 경우
            ...
            provider:
              naver:
                ...
                # 요청을 보내면 사용자에 대한 정보가 json응답으로 돌아올 때,
    ...
    //oauth의 과정이 일어난 다음, 사용자 정보를 받아온 상태에서 호출되는 함수
        ...
        public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
            ...
            String userNameAttributeName = userRequest
                    ...
                    //정의했떤 yml파일에 provider 정보를 받아오는 과정
                    .getProviderDetails()
                    //유저 정보를 어디에서 받아 오는지
...
4. WebSecurityConfig 설정
    ...
    ```java
    .and()
    // 소셜로그인이 되었다면 로컬 로그인을 한 것처럼 취급하겠다.
    .oauth2Login()
    .userInfoEndpoint()
...
6. 반환값 설정
    ...
    //oauth의 과정이 일어난 다음, 사용자 정보를 받아온 상태에서 호출되는 함수
        ...
        public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
            ...
            String userNameAttributeKey = userRequest
                    // yml에 정의했던 registration uri에 대해서 어떤 uri를 기준으로 들어온 것인지 묻는 과정
                    // - 현재 스레드에서 진행된 요청이 어떤 uri를 기준으로 진행된 건지
                    .getClientRegistration()
                    //정의했떤 yml파일에 provider 정보를 받아오는 과정
...
## AuthServerConfig, WebSercutiryConfig
